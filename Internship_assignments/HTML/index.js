...
const link = 'https://github.com/UkeshThapa/Outside/blob/master/Internship_assignments/HTML/'
...
for (let i = 0; i < 3; i++) {
    ...
    createHeading('h2','sub_title',pages[i],`.assign${i}`)
    createEmptyDiv('div',`btn_wrapper wrapper${i}`,`.assign${i}`)
    createAnchor(`link_assignment_demo${i}`,`${pages[i]}.html`, `.wrapper${i}`)
    createButton('btn', 'Demo',`.link_assignment_demo${i}`)
    createAnchor(`link_assignment_code${i}`,`${link}${pages[i]}.html`,`.wrapper${i}`)
